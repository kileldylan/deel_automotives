...
const ExploreSedan = () => {
  ...
  useEffect(() => {
    const fetchCarData = async () => {
      try {
        const { data, error } = await supabase.from('cars').select('*').eq('type', 'Sedan');
  ...
  return (
    <>
      <CustomAppBar />
      <Container sx={{ py: 4 }}>
        <Typography variant="h4" sx={{ textAlign: 'center', mb: 4 }}>Explore Our Sedans</Typography>
